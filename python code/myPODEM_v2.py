from mygraph_v2 import G
from operator import itemgetter 
import gates_lib
import os
os.remove("PODEM_output.txt")
print("File Removed!")
...
for item in G.edges(data=True):
	if (item[0]!='PI'):
		for i in range(2):
			if(i == 0 ):
				faulty_edge_list = [item[0],item[1],'sa0','1']				
			elif(i == 1):
				faulty_edge_list = [item[0],item[1],'sa1','0']
			print "------------------------------faulty node ------------------------------------------------"
			print  "fault",fault_count,"==", faulty_edge_list
			D_fronteir_list=[]
			count = 0;
			fault_count=fault_count+1
			implications_stack=[]
			untestable_flag = 0
	#		G.add_edge(faulty_edge_list[0],faulty_edge_list[1], value_non_fault='x',value_faulty='x',fault=faulty_edge_list[2],CO='x')
			G.edges[faulty_edge_list[0],faulty_edge_list[1]]['fault']=faulty_edge_list[2]
			for edge in G.edges(data=True):
					print edge[0],edge[1],"fault",edge[2]['fault']
